...
dist:
	...
	ln -sf $(APP) dist/tmp/$(APP)-darwin-arm64/$(APP_ALIAS)
	cd dist/tmp/$(APP)-darwin-arm64 && tar -czf ../../$(APP)-$(ASSET_VERSION)-darwin-arm64.tar.gz $(APP) $(APP_ALIAS)
	mkdir -p dist/tmp/$(APP)-darwin-amd64
	GOOS=darwin GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o dist/tmp/$(APP)-darwin-amd64/$(APP) .
	ln -sf $(APP) dist/tmp/$(APP)-darwin-amd64/$(APP_ALIAS)
	cd dist/tmp/$(APP)-darwin-amd64 && tar -czf ../../$(APP)-$(ASSET_VERSION)-darwin-amd64.tar.gz $(APP) $(APP_ALIAS)
	mkdir -p dist/tmp/$(APP)-linux-amd64
	GOOS=linux GOARCH=amd64 go build -ldflags "$(LDFLAGS)" -o dist/tmp/$(APP)-linux-amd64/$(APP) .
	ln -sf $(APP) dist/tmp/$(APP)-linux-amd64/$(APP_ALIAS)
	cd dist/tmp/$(APP)-linux-amd64 && tar -czf ../../$(APP)-$(ASSET_VERSION)-linux-amd64.tar.gz $(APP) $(APP_ALIAS)
	mkdir -p dist/tmp/$(APP)-linux-arm64
	GOOS=linux GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o dist/tmp/$(APP)-linux-arm64/$(APP) .
	ln -sf $(APP) dist/tmp/$(APP)-linux-arm64/$(APP_ALIAS)
	cd dist/tmp/$(APP)-linux-arm64 && tar -czf ../../$(APP)-$(ASSET_VERSION)-linux-arm64.tar.gz $(APP) $(APP_ALIAS)
	...
	cp dist/tmp/$(APP)-windows-amd64/$(APP).exe dist/tmp/$(APP)-windows-amd64/$(APP_ALIAS).exe
	cd dist/tmp/$(APP)-windows-amd64 && zip -q ../../$(APP)-$(ASSET_VERSION)-windows-amd64.zip $(APP).exe $(APP_ALIAS).exe
	rm -rf dist/tmp
