...
const aliasDest = path.join(binDir, ALIAS + (isWindows ? ".exe" : ""));
...
function installAlias() {
  ...
  if (isWindows) {
    ...
    try {
      fs.symlinkSync(binaryName, aliasDest);
    } catch (_) {
      fs.copyFileSync(dest, aliasDest);
    }
  }
...
function install() {
  ...
  try {
    ...
    installAlias();
    console.log(`${NAME}/${ALIAS} v${VERSION} installed successfully`);
  } finally {
